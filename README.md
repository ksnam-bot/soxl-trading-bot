...
`docs/positions.html`에서 실제 보유 티어(매수일/매수가/수량/목표가/정리예정일), 현금,
대기 중인 매수 주문을 직접 추가·수정·삭제할 수 있습니다. 봇이 계산한 것과 실제 체결이
다를 때(주문 실수, 부분 체결, 수동 매매 등) 여기서 바로잡으면 됩니다. 설정값 편집과 같은
GitHub 개인 액세스 토큰을 그대로 씁니다 — 이미 설정값 탭에서 연결했다면 다시 안 해도 됩니다.
저장하면 `state/<계좌id>.json`이 바로 갱신되고, 다음 자동 실행부터 이 값을 기준으로 계산됩니다.
